...
glm::vec3 Bone::CalcInterpolatedPosition(float time)
{
	...
	std::cout << "Position keys: " << this->animNode->mNumPositionKeys << std::endl;
...
glm::quat Bone::CalcInterpolatedRotation(float time)
{
	...
	std::cout << "Rotation keys: " << this->animNode->mNumRotationKeys << std::endl;
	...
	float DeltaTime = this->animNode->mRotationKeys[NextRotationIndex].mTime - this->animNode->mRotationKeys[LastRotationIndex].mTime; //Time between frames
	float Factor = (time - (float)animNode->mRotationKeys[LastRotationIndex].mTime) / DeltaTime; //Percentage of the way between frames
