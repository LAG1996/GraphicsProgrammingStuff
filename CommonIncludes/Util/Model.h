...
public:
	...
	void Draw(Shader shader, glm::vec3 Color, float shininess, float deltaTime, bool isinstance, int count = 0)
	{
		if (is_rigged)
		{
			Update(deltaTime);
		...
		{
			this->_meshes[i].Draw(shader, Color, shininess, isinstance, count);
		}
	}
